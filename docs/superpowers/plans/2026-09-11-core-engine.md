...
> не нужно: положите рядом сборку с libass и укажите её переменной
> `SUBTITLER_FFMPEG` (и при желании `SUBTITLER_FFPROBE`) — их читает
> `ProcessFfmpegRunner.fromEnvironment()`, который используют и тесты,
> и CLI. Опции вида `--with-libass` у Homebrew давно убраны, такая команда
> завершится ошибкой `invalid option`.
