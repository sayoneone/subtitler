...
> **Что реально понадобилось (выполнено 2026-09-11).** Задача сработала как
> задумано — конфликты тулчейна вылезли сразу и были починены до написания
> логики. Отклонения от текста выше:
> - Gradle wrapper пришлось поднять с 9.1.0 до **9.4.1**: AGP 9.2.1 отказывается
>   работать со старым (`android/gradle/wrapper/gradle-wrapper.properties`).
> - macOS-таргет поднят с 10.15 до **11.0** (`macos/Podfile` и
>   `MACOSX_DEPLOYMENT_TARGET` в `project.pbxproj`) — иначе не собирается `gal`.
>   На релизные платформы это не влияет: macOS у нас только для отладки.
> - `flutter_lints` — **^6.0.0**, `environment.sdk` — **^3.12.2** (то, что даёт
>   актуальный Flutter 3.44.6).
> - `media_kit` взят с pub.dev (^1.2.6), а не git-пином на main. Все 95
>   зависимостей разрешились без конфликтов. Пин на main понадобится, только
>   если при разработке интерфейса (план 2) начнёт мешать краш hot-restart.
> - Android SDK Platform 37 и Build-Tools 36 доустановились автоматически.
...
> ложный успех здесь опаснее пропуска. Проверяемыми без libass остаются
> счётчик светлых пикселей и то, что копия видео без субтитров проверку
> не проходит.
>
> Чтобы проверить вшивание на macOS по-настоящему, системный ffmpeg менять
> не нужно: `brew install ffmpeg-full` (формула из homebrew-core, та же
> версия, но с libass, keg-only — ничего не подменяет), затем указать её
> переменной `SUBTITLER_FFMPEG` — её читает
> `ProcessFfmpegRunner.fromEnvironment()`, который используют и тесты, и CLI.
> Опции вида `--with-libass` у Homebrew давно убраны из формул core, а в
> кране `homebrew-ffmpeg` такого флага и не было: libass там подключается
> безусловно, поэтому команда падает с `invalid option`.
>
> Порог проверки видимости (`subtitleMinNewPixels`) подобран замером
> 2026-09-11 на ffmpeg 9.0.1 + libass 0.17.5: перекодирование яркого ролика
> 848x480 без субтитров двигает счётчик светлых пикселей примерно на 60,
> а одна реальная реплика добавляет около 1100. Исходные 300 из первой
> редакции плана были взяты на глаз и отсекали короткие реплики.
