...
/// Реплика попадает в кадр, только если ей есть что сказать по-русски.
///
/// `empty` — ffmpeg нашёл речь, а распознавание ничего не вернуло: в кадре
/// нечего показать, даже если текст случайно остался. Пустой перевод не
/// показывается по той же причине, по какой `buildSrt` его пропускает, —
/// иначе предпросмотр разошёлся бы с тем, что будет вшито.
bool isCueVisible(Cue cue) =>
    cue.status != CueStatus.empty && cue.ru.trim().isNotEmpty;
