...
///
/// - На -aman/-eman «я …-ю»: aman «ой, осторожно», (kos)kocaman
///   «огромный», kahraman «герой», yaman «ловкий», eleman «работник;
///   парень» и слитное herzaman (правильно her zaman «всегда», но в
///   частотном списке слитное написание встречается 2093 раза).
...
///   morga «в морг».
/// - На причастие -gan: kaygan «скользкий», yorgan «одеяло», organ
///   «орган», saldırgan «нападавший» — в записях о нападениях слово
///   обычное.
/// - На вопрос -dimi: kendimi «себя», adımı «моё имя», derdimi «мою беду»,
///   yardımı «помощь» (вин. п.; «его помощь»), разговорное dimi «не так
///   ли?» (değil mi).
///
/// Список сверен перебором всех 10 000 слов частотного списка турецких
/// субтитров («Turkish WordList 10K» Викисловаря, 92 млн слов): кроме
/// перечисленных, узбекское окончание там получают только имена из
/// зарубежных фильмов (Morgan, Logan, Megan) — в живой речи они редки и
/// не внесены.
/// Не внесены и редкие в речи слова (saman, keman, gaga, slogan, hakka,
/// turizmi). alıngan «обидчивый» тоже: так турецкая модель может записать
/// узбекское olingan «взятый».
const List<String> _turkishLookalikes = [
  r'^(koskoc|koc|kahr|y|herz)?aman$',
  r'^eleman$',
  r'[^aeiou]laman$',
  r'maman$',
  r'^(kav|dal|kar|mor)ga$',
  r'^(kay|yor|or|saldir)gan(i|da|dan|lar)?$',
  r'^(kend|ad|derd|yard)imi$',
  r'^dimi$',
];
...
/// (по-турецки ham — «сырой», kel — «лысый», mana — «смысл», hop —
/// междометие «оп», ahır — «хлев») и узбекское de «скажи» против самого
/// частого турецкого de. В речи на своём языке они звучат несравнимо
/// чаще, чем у соседа, и остаются признаком языка.
///
...
/// а bırak — 133. В частотном списке турецких субтитров («Turkish WordList
/// 10K» Викисловаря) sal — 6264-е, bırak — 219-е.
///
/// И так же оставлены узбекские shular «эти» и ancha «довольно, намного»,
/// хотя по скелету они совпадают с турецкими sular «воды» и anca
/// («столько», «так»; ещё — то же, что ancak «только», по словарю ТДК
/// sozluk.gov.tr). В том же списке субтитров
/// sular — 9260-е, anca — 9392-е, около 1100 употреблений на 92 млн слов,
/// а shular и ancha — служебные слова узбекской речи. Общими их сделать
/// нельзя: турецкая модель пишет услышанное shular как şular, а ancha —
/// как ança, скелеты у этих записей те же sular и anca, и признак
/// узбекской речи пропал бы. Цена решения: турецкие sular и anca у
/// турецкой модели считаются «чужими» словами.
