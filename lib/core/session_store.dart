...
class SessionStore {
  ...
  /// а ролик пришлось бы оплачивать заново. Если рядом с видео сессии
  /// нет, читаются и файлы под чужим хешем: путь к тому же видео мог
  /// смениться (см. [_otherPathFallbacks]).
  ...
  /// рядом с видео сессии нет (см. [_ownOrMoved]).
  ///
  ...
  /// Сессии прежних схем читаются (см. [Session.fromJson]). Если рядом с
  /// видео сессии нет, в запасной папке среди прочих ищется и сессия,
  /// записанная, когда путь к видео был другим (см. [_ownOrMoved]).
  ///
  ...
  /// [own] — файл рядом с видео, запасной под своим хешем и под прежним
  /// именем, именно в таком порядке; [moved] — запасные под чужим хешем.
  ///
  /// Если рядом с видео есть подходящая сессия, файлы под чужим хешем не
  /// читаются: это рабочая копия вещдока, в которую можно писать, и её
  /// правки — свои. Файл под чужим хешем может быть другой копией того же
  /// вещдока (оригинал открывали с защищённой флешки): выбирай «самый
  /// свежий» из всех сразу, рабочая копия открылась бы с текстом
  /// оригинала, а первая же правка затёрла бы её собственный файл.
  ///
  /// Рядом с видео сессии нет — видео на носителе без записи, и все его
  /// сессии лежат в запасной папке. Тогда запасной файл под своим хешем и
  /// файл прежней версии — такие же кандидаты, как файлы под чужим:
  /// берётся самый свежий. Иначе флешка, вставленная то под одной
  /// буквой, то под другой, открывалась бы со старым текстом, а после
  /// обновления с 0.1.x — с текстом той версии, и правки, сделанные под
  /// другой буквой, были бы не видны.
  Future<Session?> _ownOrMoved(
    ...
    final beside = await _loadFreshest(videoPath, [own.first], actual);
    if (beside != null) return _loadFreshest(videoPath, own, actual);
    return _loadFreshest(videoPath, [...own.skip(1), ...moved()], actual);
  }
